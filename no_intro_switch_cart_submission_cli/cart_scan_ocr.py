...
_VLM_CROP_DEBUG_ROLES: tuple[str, ...] = ("insert_spread", "cart_front", "cart_back")


def vlm_debug_crops_requested(cfg: dict[str, Any], args: Any) -> bool:
    """True when CLI or config asks to run ``vlm_extract_command`` on ``_ocr_crop_debug`` ROI PNGs."""
    if getattr(args, "vlm_debug_crops", False):
        return True
    block = cfg.get("scan_ocr")
    if isinstance(block, dict):
        v = block.get("vlm_debug_crops", False)
        if v is False or str(v).strip().lower() in ("0", "false", "no"):
            return False
        return bool(v)
    return False
...
def run_vlm_on_ocr_crop_debug_for_cli(
    release_dir: Path,
    serial_row: dict[str, str],
    cfg: dict[str, Any],
    args: Any,
    *,
    ...
    """
    For each scan role, run ``scan_ocr.vlm_extract_command`` on ``<release>/_ocr_crop_debug/<role>_r0.png``
    (first ROI crop from ``--ocr-dump-crops``) and merge JSON into ``serial_row`` like normal VLM merge.
    """
    if not vlm_debug_crops_requested(cfg, args):
        return []
    if not vlm_extract_command(cfg):
        hint = (
            "vlm_debug_crops: skipped — --vlm-debug-crops only runs your "
            "scan_ocr.vlm_extract_command argv on _ocr_crop_debug PNGs; "
            "it does not start LM Studio or ship a model. Add that argv list "
            "under scan_ocr (see no_intro_submit.example.json and README)."
        )
        if config_path is not None:
            try:
                rp = str(config_path.resolve())
            except OSError:
                rp = str(config_path)
            exists = config_path.is_file()
            hint += f" Loaded config path: {rp}" + (
                "" if exists else " (file missing — JSON treated as empty)."
            )
        return [hint]
    dbg = release_dir / "_ocr_crop_debug"
    if not dbg.is_dir():
        return [
            "vlm_debug_crops: skipped (_ocr_crop_debug missing; run once with --ocr-scans --ocr-dump-crops)"
        ]
    fill_empty = vlm_fill_empty_only(cfg)
    lines: list[str] = []
    for role in _VLM_CROP_DEBUG_ROLES:
        png = dbg / f"{role}_r0.png"
        if not png.is_file():
            lines.append(f"vlm_debug_crops: skip {role} (missing {png.name})")
            continue
        vlm_out, err = run_vlm_serial_extract(png, cfg, role=role)
        if err:
            lines.append(f"vlm_debug_crops: {role}: {err}")
            continue
        before = {k: (serial_row.get(k) or "").strip() for k in sorted(_VLM_JSON_KEYS)}
        serial_row.update(
            merge_vlm_serial_fields(serial_row, vlm_out, fill_empty_only=fill_empty)
        )
        after = {k: (serial_row.get(k) or "").strip() for k in sorted(_VLM_JSON_KEYS)}
        changed = [k for k in sorted(_VLM_JSON_KEYS) if before.get(k) != after.get(k)]
        ch = f" → updated {', '.join(changed)}" if changed else " (no new values)"
        lines.append(f"vlm_debug_crops: {role} OK{ch}")
    return lines
